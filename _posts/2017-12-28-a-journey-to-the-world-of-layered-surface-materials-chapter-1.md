...
An interesting property of their single-scattering approximation is (and the paper doesn’t explain this clearly enough) that they use the micro-facet’s normal for computing the refracted directions. This may cause approximation imprecisions when the normal approaches grazing angles.
...
<p style="text-align: center">
   ...
   Geometry of BSDF sampling. Image reused from the Weidlich-Wilkie paper.
</p>
...
For the reflection path, we only generate the reflected direction and we are done.

For the refraction path, we have to generate a refracted direction using the micro-facet normal, and use it to sample the direction of reflection from inner layer. The resulting reflected direction has to be again refracted from inside through the micro-facet of the outer layer.

#### PDF

The paper doesn't properly explain how to compute the resulting PDF when sampling the BSDF.

When evaluating the whole BSDF, the PDF is computed as a weighted sum of the individual PDFs $p_i$ of both layers with constant weights $w_i$:
...
I find the sampling process of the paper a little problematic. It basically randomly picks one of the layers according to probabilities equal to constants $w_i$ and uses its PDF to generate the resulting direction. Since the sampling strategy should be as much proportional to the whole BSDF as possible we need to weight the individual sampling PDFs with the actual contributions of the respective BSDF components (layers). However, using constant weights hardly leads to an optimal sampling strategy since it completely ignores both the Fresnel attenuation and the attenuation within the medium which are both direction dependent.

The second problem with the sampling arises from the fact that the probability density of the direction generated by the inner BSDF changes when undergoing refraction operation on the outer layer. This leads to inconsistency between the computed and the actual PDF, which, in turn, leads to a biased Monte-Carlo estimator in the renderer.